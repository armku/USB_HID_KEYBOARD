...
typedef struct _ENDPOINT_INFO
{
  ...
  */
  uint16_t  Usb_wLength;					  //还有多少数据需要操作
  uint16_t  Usb_wOffset;					  //当前数据缓冲的偏移
  uint16_t  PacketSize;						  //端点支持的包长度
  uint8_t   *(*CopyData)(uint16_t Length);	 //主要是指明数据长度、返回数据缓冲指针的
}ENDPOINT_INFO;
...
typedef struct _DEVICE_PROP
{
  ...
  */
  RESULT (*Class_NoData_Setup)(uint8_t RequestNo);	
							   //有些类特殊请求，usb_core.c并不会支持，而交由上层用户处理。
